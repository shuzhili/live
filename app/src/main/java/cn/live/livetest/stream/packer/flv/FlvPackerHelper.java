 ...
 * @Package com.devyk.stream.packer.flv
 * @Description:
 * @Author Jim
 * @Date 16/9/23
 * @Time 下午6:34
...
public class FlvPackerHelper {
    ...
    public static void writeFlvTagHeader(ByteBuffer buffer, int type, int dataSize, int timestamp) {
         ...
         */
        //tag前四个字节是 1字节type 3字节size  type=1bit(reserved)+2bit(filter)+5bit(音频0x8 视频0x9 脚本0x12)
        ...
        buffer.putInt(time);//时间戳:时间戳+扩展时间戳
        buffer.put((byte) 0);//9-11 streamId
        buffer.put((byte) 0);//9-11 streamId
        buffer.put((byte) 0);//9-11 streamId
    }
     ...
     * @return
     */
    //buffer FlvVideoFrameType.KeyFrame, FlvVideoCodecID.AVC, FlvVideoAVCPacketType.SequenceHeader
    ...
    public static void writeFirstAudioTag(ByteBuffer buffer, int audioRate, boolean isStereo, int audioSize) {
        ...
        //3--->0011 0x7---->0111  0001&0111--->0001   (0001 0000)|(0000 0001)=(0001 0001)
        audioInfo[0] = (byte) (0x10 | ((soundRateIndex>>1) & 0x7));
        //0011&0001--->0001   0000 0001<<7->0100 0000
    ...
    public static void writeAudioHeader(ByteBuffer buffer, boolean isFirst, int audioSize) {
        ...
        header[0] = (byte)(
                ((byte) (soundFormat & 0x0F) << 4) |
                ((byte) (soundRateIndex & 0x03) << 2) |
                        ((byte) (soundSize & 0x01) << 1) |
                        ((byte) (soundType & 0x01)));
